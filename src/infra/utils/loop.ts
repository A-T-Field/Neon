 ...
 * @LastEditTime: 2022-04-23 17:43:18
...
import {
    ...
    isString,
    isObject,
} from '@neon-utils/checkers';

type EachArrayIteratore<T> = (currentValue: T, key: number) => void | boolean;

type EachStringIteratore = (
    currentValue: string,
    key: number
) => void | boolean;

type EachObjectIteratore<T = any> = (
    currentValue: T,
    key: string
) => void | boolean;

type MapArrayIteratore<TItem, TResult> = (
    currentValue: TItem,
    key: number
) => TResult;

type MapSringIteratore<TResult> = (
    currentValue: string,
    key: number
) => TResult;

type MapObjectIteratore<TItem, TResult> = (
    currentValue: TItem,
    key: string
) => TResult;

export const toArray = (val: any): Array<any> =>
    isArray(val) ? val : isValid(val) ? [val] : val;
...
export function each<T extends {}, TValue extends T[keyof T]>(
    val: T,
    iterator: EachObjectIteratore<TValue>
): void;
...
export function map<TItem, TResult>(
    val: Array<TItem>,
    iterator: MapArrayIteratore<TItem, TResult>
): Array<TResult>;
export function map<T extends {}, TResult>(
    val: T,
    iterator: MapObjectIteratore<T[keyof T], TResult>
